...
public class ResourceManager {
    ...
    public static void preload(final Dimension windowSize) {
    	System.out.println("ResourceManager.preload()");
    ...
    public static void startBackgroundPreloading(final Dimension windowSize) {
    	System.out.println("ResourceManager.startBackgroundPreloading()");
        lastWindowSize = windowSize;
        if (dirty) {
            updateIfDirty();
...
//                }
            	...
            	List<Resource> resources =
                    new LinkedList<Resource>(mergedContainer.getResources().values());
                for (Entry<String, Resource> e : mergedContainer.getResources().entrySet()) {
                    if (preloadThread != this) {
                        return;
                    }
                    if (e.getValue() == null) {
                    	System.err.println("Resource not loaded: " + e.getKey());
                    }
                    e.getValue().preload();
                }
            }
